<template>
  <div class="min-h-screen bg-gray-50 py-10 px-4 sm:px-6 lg:px-8 font-sans text-gray-900">
    <div class="w-full max-w-4xl mx-auto">
      ...
      <!-- Back Navigation -->
      <NuxtLink to="/events" class="inline-flex items-center text-sm font-medium text-gray-500 hover:text-gray-900 mb-6 transition-colors">
      ...
      <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <div class="px-8 py-6 border-b border-gray-100 flex justify-between items-center">
          <div>
            <h2 class="text-2xl font-bold text-gray-900">Modify Event</h2>
            <p class="text-gray-500 text-sm mt-1">Update event details, status, and media.</p>
          </div>
        </div>

        <div v-if="isLoadingData" class="p-20 flex justify-center">
          ...
          <form @submit.prevent="handleUpdateEvent" class="space-y-6">
                ...
                <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-lg hover:bg-gray-50 transition-colors">
                  <div class="space-y-1 text-center">
                    <svg v-if="!imagePreviewUrl" class="mx-auto h-12 w-12 text-gray-400" stroke="currentColor" fill="none" viewBox="0 0 48 48" aria-hidden="true">
                      <path d="M28 8H12a4 4 0 00-4 4v20m32-12v8m0 0v8a4 4 0 01-4 4H12a4 4 0 01-4-4v-4m32-4l-3.172-3.172a4 4 0 00-5.656 0L28 28M8 32l9.172-9.172a4 4 0 015.656 0L28 28m0 0l4 4m4-24h8m-12 4h.02" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                    </svg>
                    ...
                    <div class="flex text-sm text-gray-600 justify-center">
                      <label for="file-upload" class="relative cursor-pointer bg-white rounded-md font-medium text-blue-600 hover:text-blue-500 focus-within:outline-none">
                        <span>{{ imageFile ? 'Change image' : (imagePreviewUrl ? 'Update image' : 'Upload a file') }}</span>
            ...
            <div class="pt-6 mt-8 border-t border-gray-100 flex items-center justify-between">
              ...
              >
                ...
                <span>{{ eventStore.isLoading ? 'Saving...' : 'Save Changes' }}</span>
              </button>
            </div>
          </form>
        </div>
...
const eventForm = ref<Partial<EventCreate>>({
  title: '',
  description: '',
  location: '',
  start_at: '',
...
watch(() => authStore.isProfileLoaded, async (loaded) => {
  if (loaded) {
    if (!eventId) {
      router.push('/events')
      return
    ...
    // Ensure events are loaded so we can find it
    if (eventStore.events.length === 0) {
      await eventStore.fetchEvents()
    }

    const existingEvent = eventStore.events.find(e => e.id === eventId)
    if (existingEvent) {
      eventForm.value = {
        title: existingEvent.title,
        description: existingEvent.description,
        location: existingEvent.location,
        start_at: formatForInput(existingEvent.start_at),
        end_at: formatForInput(existingEvent.end_at),
        category: existingEvent.category,
        capacity: existingEvent.capacity,
        status: existingEvent.status,
      }
      
      if (existingEvent.media_assets && existingEvent.media_assets.length > 0) {
        imagePreviewUrl.value = existingEvent.media_assets[0].file_url
      }
    } else {
...
const handleUpdateEvent = async () => {
  ...
  try {
    const updatedEvent = await eventStore.updateEvent(eventId, eventForm.value, imageFile.value)
    ...
    if (updatedEvent) {
      ...
      await eventStore.fetchEvents() // refresh global state
      ...
      messageType.value = 'error'
      message.value = 'Failed to update event. Please try again.'
    }
  } catch (err: any) {
    messageType.value = 'error'
    message.value = err?.message || 'An error occurred while updating the event.'
  }
}
</script>
